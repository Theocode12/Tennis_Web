...
config = load_config()
MEDIA_ROOT = config.get("media", "root_dir")
...
class MusicService:
    ...
    async def register_song(self, song_id: str) -> Song:
        ...
        if not self.file_store.song_exists(song_id):
            logger.warning(f"Song '{song_id}' does not exist on disk")
            raise ValueError(f"Song '{song_id}' does not exist on disk")

        meta = self.file_store.get_metadata(song_id)
        if not meta:
            logger.warning(f"Metadata missing for song '{song_id}'")
        ...
        song_data = {
            ...
            "hls_path": f"/{MEDIA_ROOT}/{song_id}/playlist.m3u8",
        }
