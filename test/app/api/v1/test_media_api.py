...
async def test_list_registered_songs(mock_media_env, async_client, redis_client):
    ...
    response = await async_client.get("/api/v1/music/songs", headers=_secret_headers())
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["id"] == mock_media_env["song_id"]
...
async def test_scan_media_folder(mock_media_env, async_client):
    ...
    response = await async_client.get("/api/v1/music/media-folder", headers=_secret_headers())
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    assert data[0]["id"] == mock_media_env["song_id"]
...
async def test_ingest_song_endpoint(async_client, redis_client):
    ...
    with patch("app.api.v1.media.process_song_task.delay") as mock_delay:
        ...
        response = await async_client.post("/api/v1/music/ingest", data=data, files=files, headers=_secret_headers())
...
async def test_get_ingest_status(async_client, redis_client):
    ...
    response = await async_client.get(f"/api/v1/music/ingest/{job_id}", headers=_secret_headers())
    assert response.status_code == 200
...
async def test_register_song_endpoint(mock_media_env, async_client, redis_client):
    ...
    response = await async_client.post(f"/api/v1/music/register/{song_id}", headers=_secret_headers())
    assert response.status_code == 200
...
async def test_make_playlist_endpoint(async_client, redis_client):
    ...
    response = await async_client.post("/api/v1/music/playlist", data={"name": "New P"}, headers=_secret_headers())
    assert response.status_code == 200
...
async def test_insert_song_to_playlist_endpoint(mock_media_env, async_client, redis_client):
    ...
    # Register first
    await async_client.post(f"/api/v1/music/register/{sid}", headers=_secret_headers())

    response = await async_client.post(f"/api/v1/music/playlist/{pid}/add/{sid}", headers=_secret_headers())
    assert response.status_code == 200
