...
async def test_handle_start_stream_success(mock_app):
    ...
    # Verify manager was called with a SchedulerContext for the right match
    call_args = mock_manager.create_or_get_scheduler.call_args
    assert call_args is not None
    context = call_args.args[0] if call_args.args else call_args.kwargs.get("context")
    assert isinstance(context, SchedulerContext)
    assert context.game_id == "match-123"
