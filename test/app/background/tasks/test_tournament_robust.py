...
# Inline test config (replaces the old hardcoded competition file)
TEST_PLAYERS = [
    "Alcaraz C.",
    "Tiafoe F.",
    "Shelton B.",
...
TEST_PLAYER_METADATA = {p: {"name": p, "world_ranking": i + 1} for i, p in enumerate(TEST_PLAYERS)}

TENNIS_TOURNAMENT_CONFIG = {
    "name": "Test Open",
    "season": 2026,
    "surface": "Hard",
    "level": "Grand Slam",
    ...
    "player_metadata": TEST_PLAYER_METADATA,
}
...
def mock_dependencies():
    with (
        patch("app.domain.tournament.tennis.get_logger") as mock_get_logger,
        patch("app.domain.tournament.tennis.CONFIG", {"some": "config"}),
...
def test_build_match_payload(mock_dependencies):
    tournament = Tournament("test", TENNIS_TOURNAMENT_CONFIG)
    match_id = "match-123"
    pa, pb = "Alcaraz C.", "Tiafoe F."

    payload = tournament._build_match_payload(match_id, pa, pb)

    assert payload["game_id"] == match_id
    # Players now include world_ranking from metadata
    assert payload["players"] == [
        [{"name": "Alcaraz C.", "world_ranking": 1}],
        [{"name": "Tiafoe F.", "world_ranking": 2}],
    ]
    assert payload["match_context"]["tournament"]["tournament_id"] == "test"
    assert payload["match_context"]["tournament"]["round"]["code"] == "R16"


def test_build_match_payload_without_metadata(mock_dependencies):
    """Players not in metadata still get name-only dicts."""
    config = {k: v for k, v in TENNIS_TOURNAMENT_CONFIG.items() if k != "player_metadata"}
    config["players"] = ["p1", "p2"]
    config["rounds"] = [{"code": "F", "name": "Final", "required_players": 2}]

    tournament = Tournament("test", config)
    payload = tournament._build_match_payload("m1", "p1", "p2")

    assert payload["players"] == [[{"name": "p1"}], [{"name": "p2"}]]
...
async def test_full_tournament_progression(mock_dependencies):
    ...
    with patch.object(Tournament, "_run_match", side_effect=mock_run_match):
        ...
        # First player always wins with mock_run_match
        assert tournament.winner == TEST_PLAYERS[0]
