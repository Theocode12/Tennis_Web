...
def mock_dependencies(redis_client, redis_client_sync, tmp_path):
    ...
    temp_upload = tmp_path / "uploads_test"
    temp_store = tmp_path / "music_test.json"
    temp_media.mkdir(exist_ok=True)
    temp_upload.mkdir(exist_ok=True)

    # Build a ConfigParser that redirects all path-based config keys to tmp_path.
    # This covers dependencies.get_music_service() and ingestion_service.process_song(),
    # both of which call load_config() at runtime instead of using module-level constants.
    ...
    mock_config["media"] = {
        "root_dir": str(temp_media),
        "music_store_path": str(temp_store),
        "upload_dir": str(temp_upload),
    }
    ...
    with (
        ...
        # Patch load_config wherever it is imported so all path lookups use tmp_path
        patch("app.dependencies.load_config", return_value=mock_config),
        patch("app.services.ingestion_service.load_config", return_value=mock_config),
        # app.api.v1.media still uses a module-level UPLOAD_DIR constant
        patch("app.api.v1.media.UPLOAD_DIR", temp_upload),
    ):
        dependencies.music_service_instance = None
        dependencies.get_app_config.cache_clear()
        yield
        dependencies.music_service_instance = None
        dependencies.get_app_config.cache_clear()
...
def mock_media_env(tmp_path: Path):
    ...
    cfg = ConfigParser()
    cfg["media"] = {
        "root_dir": str(media_dir),
        "music_store_path": str(store_file),
        "upload_dir": str(tmp_path / "uploads"),
    }

    with (
        patch("app.dependencies.load_config", return_value=cfg),
