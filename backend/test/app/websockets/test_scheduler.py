...
from app.scheduler.game_feeder import FileGameFeeder
...
async def consume_messages(
    generator: asyncio.StreamReader, count: int, timeout: float = 1.0
) -> list:
    messages = []
...
class TestGameScheduler(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        self.feeder = FileGameFeeder(
            game_id=TEST_GAME_ID_SCHED, storage=self.storage
        )
    ...
    async def asyncTearDown(self):
        ...
        if self.scheduler_task and not self.scheduler_task.done():
            ...
            try:
                ...
                print(
                    f"Error during scheduler task cleanup: {e}"
                )  # Log other errors
    ...
    def test_initialization(self):
        ...
        self.assertFalse(
            self.scheduler.pause_event.is_set(),
            "Scheduler should be paused initially",
        )
    ...
    async def test_run_publishes_scores_when_started(self):
        ...
        score_subscriber = self.broker.subscribe(
            TEST_GAME_ID_SCHED, BrokerChannels.SCORES_UPDATE
        )
        ...
        received_scores = await consume_messages(
            score_subscriber, len(TEST_SCORES_LIST_SCHED), timeout=total_timeout
        )
        ...
        await asyncio.wait_for(
            self.scheduler_task, timeout=1.0
        )  # Should finish quickly after last score
        self.assertTrue(self.scheduler_task.done())
        self.assertIsNone(
            self.scheduler_task.exception()
    ...
    async def test_pause_and_resume(self):
        ...
        await self.scheduler.start()
        received1 = await consume_messages(
            score_subscriber, 1, timeout=self.scheduler.speed + 0.5
        )
        ...
        received_remaining = await consume_messages(
            score_subscriber, remaining_count, timeout=total_timeout
        )
    ...
    async def test_adjust_speed(self):
        ...
        await consume_messages(
            score_subscriber, 1, timeout=self.scheduler.speed + 0.1
        )
        ...
        self.assertLess(
            end_time - start_time,
            expected_time_min + 0.1,
            "Scores did not arrive faster after speed adjustment",
        )
    ...
    async def test_control_messages(self):
        ...
        await self.broker.publish(
            TEST_GAME_ID_SCHED, BrokerChannels.CONTROLS, {"type": "start"}
        )
        ...
        await self.broker.publish(
            TEST_GAME_ID_SCHED, BrokerChannels.CONTROLS, {"type": "pause"}
        )
        ...
        await self.broker.publish(
            ...
            {"type": "adjust_speed", "speed": new_speed},
        )
        await asyncio.sleep(0.05)
        self.assertEqual(self.scheduler.speed, new_speed)
        ...
        await self.broker.publish(
            TEST_GAME_ID_SCHED, BrokerChannels.CONTROLS, {"type": "resume"}
        )
        await asyncio.sleep(0.05)
        self.assertTrue(self.scheduler.pause_event.is_set())
    ...
    async def test_feeder_cleanup_called_on_completion(self):
        ...
        await asyncio.wait_for(
            self.scheduler_task,
            timeout=len(TEST_SCORES_LIST_SCHED) * (self.scheduler.speed + 0.1),
        )
