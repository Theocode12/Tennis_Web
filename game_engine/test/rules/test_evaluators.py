...
from src.rules import RuleEvaluator, TennisRules
...
class TestRuleEvaluator(unittest.TestCase):
    ...
    def test_check_game_winner(self):
        # Arrange
        self.mock_rules.get_game_rule().has_player_won_game.return_value = True
        ...
        self.mock_rules.get_game_rule().has_player_won_game.assert_called_with(
            player_points, opponent_points
        )

    def test_check_set_winner(self):
        # Arrange
        self.mock_rules.get_set_rule().has_player_won_set.return_value = True
        ...
        self.mock_rules.get_set_rule().has_player_won_set.assert_called_with(
            player_set_points, opponent_set_points, None, None, False
        )

    def test_check_match_winner(self):
        # Arrange
        player_sets_won = 3
        self.mock_rules.get_match_rule().has_player_won_match.return_value = True
        ...
        self.mock_rules.get_match_rule().has_player_won_match.assert_called_with(
            player_sets_won
        )
